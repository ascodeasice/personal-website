...
const ContactRow = ({ src, href, text, num }) => {
    ...
    return (
        <div className="grid grid-cols-[5rem_min-content] max-[450px]:grid-cols-[3rem_min-content] gap-x-4 items-center">
            ...
            <a href={href} target="_blank" className={`${linkClassName} text-custom-red font-bold underline text-5xl hover:text-maroon  transition-colors opacity-0  animation-delay-${100 + 100 * num} max-[450px]:text-2xl`}>{text ?? "Link"}</a>
        </div>
    );
}
